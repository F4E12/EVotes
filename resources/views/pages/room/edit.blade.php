<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <span class="px-3 py-1 text-sm font-semibold rounded-full 
                ...
                {{ \Carbon\Carbon::now()->between($room->start_date, $room->end_date) ? 'Ongoing' : (\Carbon\Carbon::now()->lt($room->start_date) ? 'Upcoming' : 'Ended') }}
            </span>
        </div>
    </x-slot>
    ...
    <div class="py-8 bg-gray-50 min-h-screen">
        <div class="max-w-[85rem] mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form id="edit-room-form" action="{{ route('rooms.update', ['room_id' => $room->room_id]) }}" method="POST">
                ...
                <!-- MAIN GRID: 3 Columns on XL screens -->
                    ...
                    <div class="space-y-6">
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-full">
                            ...
                            <div class="space-y-6 flex-grow">
                                <!-- Title -->
                                <div>
                                    <label for="title" class="block text-sm font-bold text-gray-700 mb-2">Room Title</label>
                                    <input id="title" type="text" name="title" 
                                           ...
                                           placeholder="e.g. 2025 Student Council Election" required autofocus />
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea id="description" name="description" rows="8" 
                                              ...
                                              placeholder="Describe the purpose of this vote, rules, or instructions..." required>{{ old('description', $room->description) }}</textarea>
                                    <p class="mt-2 text-xs text-gray-500 text-right">Visible to all participants.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-full">
                            ...
                            <div class="space-y-6 flex-grow">
                                <!-- Start Date -->
                                <div>
                                    <label for="start_date" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Start Date & Time</label>
                                    ...
                                    <p class="text-xs text-gray-400 mt-2">Voting opens at this time.</p>
                                </div>

                                <!-- End Date -->
                                <div>
                                    <label for="end_date" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">End Date & Time</label>
                                    <input id="end_date" type="datetime-local" name="end_date" 
                                           value="{{ old('end_date', $room->end_date->format('Y-m-d\TH:i')) }}" 
                                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm py-3 px-4" required />
                                    <p class="text-xs text-gray-400 mt-2">Voting closes after this time.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="flex-grow flex flex-col justify-between space-y-6">
                                <div>
                                    <p class="text-red-700 text-sm leading-relaxed">
                                        Deleting this room will <strong>permanently remove</strong> all associated data, including:
                                    </p>
                                    ...
                                    <p class="text-red-700 text-sm mt-4 font-bold">This action cannot be undone.</p>
                                </div>

                                <button type="button" onclick="if(confirm('Are you sure? This is irreversible.')) document.getElementById('delete-room-form').submit();" 
                ...
                <!-- BUFFER SECTION -->
                <!-- This empty space ensures the Danger Zone is never covered by the footer -->
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-8px_30px_rgba(0,0,0,0.1)] z-50">
        <div class="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-end gap-4">
            ...
            <!-- Added min-w-[200px] and whitespace-nowrap to prevent text cramping -->
