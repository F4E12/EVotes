<x-app-layout>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-12">
            ...
            <div class="space-y-4">
                <h3 class="text-xl font-bold text-gray-900 pl-1 border-l-4 border-blue-600 leading-none">
                    &nbsp;{{ __('My Articles') }}</h3>
                <div class = "h-8"></div>
                                            ...
                                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <div class="flex items-center gap-3">
                                                    <a href="{{ route('articles.show', $article->id) }}"
                                                        class="text-gray-600 hover:text-gray-900 font-medium">{{ __('View') }}</a>
                                                    ...
                                                    <form action="{{ route('articles.destroy', $article->id) }}"
                                                        ...
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="text-red-600 hover:text-red-900 font-medium">{{ __('Delete') }}</button>
                                                    </form>
                                                </div>
                                            </td>
                                        </tr>
            ...
            <div class="space-y-4">
                <h3 class="text-xl font-bold text-gray-900 pl-1 border-l-4 border-purple-600 leading-none">
                    &nbsp;{{ __('Community News') }}</h3>
                <div class = "h-4"></div>
                            ...
                            <article
                                ...
                                <a href="{{ route('articles.show', $article->id) }}"
                                    ...
                                    @if ($article->room)
                                        <div class="absolute top-3 left-3">
                                            <span
                                                class="px-2 py-1 bg-white/90 backdrop-blur-sm text-[10px] font-bold text-gray-800 rounded-md shadow-sm">
                                                {{ $article->room->title }}
                                            </span>
                                        </div>
                                    @endif
                                ...
                                <div class="p-5 flex flex-col flex-grow">
                                    ...
                                    <div class="flex items-center justify-between text-xs text-gray-500 mb-2">
                                        <span
                                            class="font-medium text-gray-900">{{ $article->author->name ?? __('Unknown') }}</span>
                                        <span>{{ $article->created_at->format('M d') }}</span>
                                    </div>

                                    <h4
                                        class="text-lg font-bold text-gray-900 mb-2 line-clamp-2 hover:text-blue-600 transition-colors">
                                        <a href="{{ route('articles.show', $article->id) }}">
                                            {{ $article->title }}
                                        </a>
                                    </h4>
                                    ...
                                    <div class="pt-4 border-t border-gray-100 mt-auto">
                                        <a href="{{ route('articles.show', $article->id) }}"
                                            class="text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1 group">
                                            {{ __('Read Article') }}
