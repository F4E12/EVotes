        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <article class="bg-white overflow-hidden shadow-sm sm:rounded-[2.5rem] border border-gray-200 px-4 sm:px-8">
                ...
                <div class="px-6 py-10 sm:px-10 sm:py-12">
                    ...
                    <div
                        class="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-8 border-b border-gray-100 pb-6">

                        <div class="flex items-center gap-2">
                            <div
                                class="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-xs uppercase border border-blue-200">
                                {{ substr($article->author->name ?? 'A', 0, 2) }}
                            </div>
                            <span
                                class="font-semibold text-gray-900">{{ $article->author->name ?? __('Unknown Author') }}</span>
                        </div>
                        ...
                        <div class="flex items-center gap-1.5" title="{{ $article->created_at->format('d M Y H:i') }}">
                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                        ...
                        @if ($article->room)
                            <span class="text-gray-300">&bull;</span>
                            <a href="{{ route('rooms.show', $article->room->room_id) }}"
                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-colors border border-indigo-100">
